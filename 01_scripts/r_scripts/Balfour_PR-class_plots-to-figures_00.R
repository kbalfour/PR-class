...
# Now set the working directory to the "03_figs" folder, where we will save
# copies of the plots produced below (our manuscript figures!):
setwd("./03_figs")
...
# (reference and W1):
ggplot(data=maples, aes(x=watershed, y=stem_length, fill=watershed)) +
...
# (reference and W1):
ggplot(data=maples, aes(x=watershed, y=leaf1area, fill=watershed)) +
...
# (reference and W1):
ggplot(data=maples, aes(x=watershed, y=leaf_dry_mass, fill=watershed)) +
...
# (reference and W1):
ggplot(data=maples, aes(x=watershed, y=stem_dry_mass, fill=watershed)) +
