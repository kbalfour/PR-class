---
title           : "My preregistration for the Productivity and Reproducibility assignment"
...
How does calcium addition impact leaf anatomy and plant size of *Acer saccharum*?
...
Nutrient supplementation can influence plant traits, but the impact of supplementation may depend on the nutrient requirements of a specific plant. Calcium, for example, is vital to cell growth, but plant calcium requirements are typically low. This data project investigates whether and how calcium availability influences leaf anatomy and plant size in *Acer saccharum* (sugar maple), a culturally, economically, and ecologically important tree species.
...
If calcium availability does not affect plant size and leaf anatomy, then trait data will not significantly differ between reference and calcium addition transects.

If calcium availability does affect plant size and leaf anatomy, then trait data will significantly differ between reference and calcium addition transects.
...
We will not use randomization, leaving these data open to selection bias.
...
To assure that the author is unaware of any patterns or summary statistics in the data, the data set selected for this study had not been previously analysed in any way by the author.
...
Within each of two transects, field researchers selected one individual for sampling, walked 10 steps away from the individual, then selected a second individual for sampling.
...
The sample size for this data set was 180 individual maples in each of the two transect types, reference and calcium addition.
...
The sample size appears to be arbitrary, perhaps depending on the anticipated timeline to completion for the undergraduate student, limitations to site accessibility or field work funding, etc.
...
Data collection was terminated depending on undisclosed criteria.
...
We manipulated soil calcium availability by adding calcium to one transect and not the other.
...
Measured variables include stem size, stem biomass, leaf area, and leaf dry mass.
...
No calculated indices will be considered.
...
An independent t-test will be completed for each trait measured to compare groups of maples sampled in the two transects (reference and calcium addition).
...
Transformations may be required to meet assumptions of normality.
...
No checks will be performed to determine eligibility for inclusion; outliers will be included in the analysis.
...
There are no incomplete or missing data; every variable was determined from each sample included.
...
No exploratory analyses will be completed for this study.
...
The data set "hbr_maples" used for this project is publically available from the R package "lterdatasampler", and was first accessed by the author on September 10th, 2024.
